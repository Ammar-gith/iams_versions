...
<body>
    <div class="header">
        ...
        <h3><strong>PO List Summary</strong></h3>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach ($bankNames as $bankName)
                @php
                    $np = (float) ($partnerBankTotals[$bankName] ?? 0);
                    $ag = (float) ($agenciesByBank[$bankName] ?? collect())->sum('net_dues');
                    $overallPayable += $np + $ag;
                @endphp
                <tr>
                    <td>{{ $sr++ }}</td>
                    <td>Manager {{ $bankName }}</td>
                    <td class="text-end">{{ number_format($np + $ag, 0) }}</td>
                </tr>
            @endforeach
        </tbody>
        <tfoot>
