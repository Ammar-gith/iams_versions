        ...
        <div class="card mb-4" style="padding: 0;">
            <div class="card-header col-md-12 d-flex justify-content-between align-items-center">
                <div class="d-flex align-item-center ">
                    ...
                    @php
                        switch (Route::currentRouteName()) {
                            case 'advertisements.inprogress':
                                $title = 'Inprogress Advertisements';
                                break;
                            case 'advertisements.approved':
                                $title = 'Approved Advertisements';
                                break;
                            case 'advertisements.rejected':
                        ...
                        $isInprogress = $currentRoute === 'advertisements.inprogress';
                        $isApproved = $currentRoute === 'advertisements.approved';
                        $isRejected = $currentRoute === 'advertisements.rejected';
                    @endphp
                ...
                <div class="d-flex justify-content-end mb-1">
                    @if ($isInprogress)
                        <a href="{{ route('advertisements.inprogress.export.excel', request()->query()) }}"
                            class="custom-excel-button me-2">Export Excel</a>
                        <a href="{{ route('advertisements.inprogress.export.pdf', request()->query()) }}"
                            class="custom-pdf-button">Export PDF</a>
                    @elseif($isApproved)
                        <a href="{{ route('advertisements.approved.export.excel', request()->query()) }}"
                            class="custom-excel-button me-2">Export Excel</a>
                        <a href="{{ route('advertisements.approved.export.pdf', request()->query()) }}"
                            class="custom-pdf-button">Export PDF</a>
                    @elseif($isRejected)
                        <a href="{{ route('advertisements.rejected.export.excel', request()->query()) }}"
                            class="custom-excel-button me-2">Export Excel</a>
                        <a href="{{ route('advertisements.rejected.export.pdf', request()->query()) }}"
                            class="custom-pdf-button">Export PDF</a>
                    @endif
                </div>
            </div>
                            ...
                            @foreach ($advertisements as $key => $advertisement)
                                <tr>
                                    ...
                                    @php
                                        ...
                                        $class = $statusClasses[$advertisement->status->title] ?? 'bg-warning';
                                    @endphp

                                    @if (
                                        $user->hasAnyRole(['Diary Dispatch', 'Superintendent', 'Deputy Director', 'Director General', 'Secretary']) &&
                                            ($advertisement->forwarded_by_role_id == 3 && $advertisement->forwarded_to_role_id == 4))
                                        <td class="text-center align-middle">
                                            <span class="badge rounded-pill bg-label-info">Approved<br><small>Sent to
                                                    media</small></span>
                                        </td>
                                    @else
                                        <td class="text-center align-middle">
                                    ...
                                    <td class="text-center align-middle">
                                        <div class="d-flex justify-content-center align-items-center">
                                            ...
                                            <div class="action-item custom-tooltip">
                                                <a href="{{ route('advertisements.show-inprogress', [$advertisement->id]) }}"
                                            ...
                                            {{-- Archive --}}
                                            <div class="action-item custom-tooltip">
                                                <form action="{{ route('advertisements.archive', [$advertisement->id]) }}"
                                                    method="POST" class="d-inline archive-form">
                                                    @csrf
                                                    <button type="button"
                                                        class="btn p-0 border-0 bg-transparent archive-btn">
                                            ...
                                            {{-- Forward to Media (only in in-progress) --}}
                                            @if ($isApproved)
                                                <div class="action-item custom-tooltip">
                                                    @if (
                                                        $user->hasRole('Superintendent') &&
                                                        ...
                                                        <form
                                                            action="{{ route('advertisement.media', $advertisement->id) }}"
                                                            method="POST">
                                                            @csrf
                                                            <button type="submit"
                                                            ...
                                                            <span class="tooltip-text">Send to media</span>
                                                        </form>
                                                    @else
                                                        <button class="btn p-0 border-0 bg-transparent" disabled>
                                                            <i class='bx bx-right-top-arrow-circle bx-icon text-muted'></i>
                                                            <span class="tooltip-text">Ad Sent</span>
                                                        </button>
                                                    @endif
                                                </div>
                                            @endif
                                ...
                                {{-- Tracking Modal --}}
                                <div class="modal fade" id="trackAdModal{{ $advertisement->id }}" tabindex="-1"
                                    aria-labelledby="trackAdModalLabel" aria-hidden="true">
                                                ...
                                                <div class="action-buttons d-flex gap-2">
                                                    <div class="y-hover">
                                                        <a href="javascript:void(0);" class="text-decoration-none">
                                                            <i class='bx bx-share-alt bx-modal-icons button-y'></i>
                                                        </a>
                                                    </div>
                                                    <div class="x-hover">
                                            ...
                                            <div class="modal-body">
                                                <div class="timeline-container">
                                                    @php
                                                        ...
                                                        $allLogs = $advertisement->changeLogs->sortBy('changed_at');

                                                        // Group logs by Date and Role
                                                        $logsByDate = $allLogs->groupBy(function ($log) {
                                                            return $log->role .
                                                                '_' .
                                                                \Carbon\Carbon::parse($log->changed_at)->format(
                                                                    'Y-m-d',
                                                                );
                                                        });
                                                    @endphp
                                                    ...
                                                    @forelse($logsByDate as $group => $logs)
                                                        @php
                                                            ...
                                                            $groupWorkflowLogs = $logs->whereNotNull('action');
                                                        ...
                                                        <div class="timeline-day-content">
                                                            <div
                                                                ...
                                                                @foreach ($groupWorkflowLogs as $log)
                                                                    <div class="change-card bg-white shadow-sm rounded mb-2 text-center"
