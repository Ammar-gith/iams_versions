        ...
        <div class="card mb-4" style="padding: 0;">
            <div class="card-header col-md-12 d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <h5 class="me-3">Agency Bills List</h5>
                            ...
                            @foreach ($billdetails as $billdetail)
                                @foreach ($billdetail->newspaper_titles as $index => $newspaper)
                                    <tr>
                                        ...
                                        <td>{{ $billdetail->spaces[$index] ?? '' }} =
                                            {{ $billdetail->total_spaces[$index] ?? '' }}</td>
                                        {{-- <td>{{ $billdetail->total_spaces[$index] ?? '' }}</td> --}}
                                        <td>{{ $billdetail->insertions[$index] ?? '' }}</td>
                                        <td>{{ number_format(round($billdetail->total_cost_per_newspaper[$index])) ?? '' }}
                                        </td>
                                        <td>{{ number_format(round($billdetail->kpra_2_percent_on_85_percent_newspaper[$index])) ?? '' }}
                                        </td>
                                        <td>{{ number_format(round($billdetail->kpra_10_percent_on_15_percent_agency[$index])) ?? '' }}
                                        </td>
                                        <td>{{ number_format(round($billdetail->total_amount_with_taxes[$index])) ?? '' }}
                                        </td>
                                    </tr>
                                @endforeach
                                <tr>
                                    <td colspan="4" class="text-center fw-bold">TOTAL</td>
                                    <td class="fw-bold">{{ number_format(round($billdetail->printed_no_of_insertion)) }}
                                    </td>
                                    <td class="fw-bold">{{ number_format(round($billdetail->printed_bill_cost)) }}</td>
                                    <td class="fw-bold">{{ number_format(round($billdetail->total_newspapers_tax)) }}</td>
                                    <td class="fw-bold">{{ number_format(round($billdetail->total_agency_tax)) }}</td>
                                    <td class="fw-bold">{{ number_format(round($billdetail->printed_total_bill)) }}</td>
                                </tr>
                                <tr>
                                    <td colspan="8" class="text-center fw-bold">TOTAL DUES</td>
                                    <td class="text-danger fw-bold">
                                        {{ number_format(round($billdetail->printed_bill_cost)) }}</td>
                                </tr>
                                <tr>
                                    <td colspan="8" class="text-center fw-bold">KPRA Sale<br> Tax On<br> Services</td>
                                    <td class="text-danger fw-bold">{{ number_format(round($billdetail->kpra_tax)) }}</td>
                                </tr>
                                <tr>
                                    <td colspan="8" class="text-center fw-bold">TOTAL BILL<br> AMOUNT</td>
                                    <td class="text-danger fw-bold">
                                        {{ number_format(round($billdetail->printed_total_bill)) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
