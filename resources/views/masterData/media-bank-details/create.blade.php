...
@push('content')
    ...
    <div class="row">
        <div class="card" style="padding-inline: 0; border-radius: 18px 18px 9px 9px;">
            <form action="{{ route('mediaBankDetail.store') }}" method="POST" class="card-body" style="padding: 0;">
                @csrf

                <div class="form-header flex w-full">
                    <h5 class="h5-reset-margin">New media bank detail</h5>
                </div>
                ...
                <div class="form-padding">
                    <div class="row mb-3 mx-5">
                        <div class="col-sm-6">
                            ...
                            <select name="media_type" id="media_type" class="form-select" required>
                                <option value="newspaper" @selected(old('media_type') === 'newspaper' || old('media_type') === null)>Newspaper</option>
                                <option value="agency" @selected(old('media_type') === 'agency')>Agency</option>
                            </select>
                            @error('media_type')
                        ...
                        <div class="col-sm-6">
                            ...
                            <input type="text" name="media_name" id="media_name" class="form-control"
                                value="{{ old('media_name') }}" placeholder="Optional display name" />
                            @error('media_name')
                                <span class="text-danger small">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-sm-6">
                            ...
                            <select name="newspaper_id" id="newspaper_id" class="form-select">
                                ...
                                @foreach ($newspapers as $n)
                                    <option value="{{ $n->id }}" @selected(old('newspaper_id') == $n->id)>
                                        {{ $n->title }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3 mx-5 d-none" id="ag_row">
                        <div class="col-sm-6">
                            ...
                            <select name="agency_id" id="agency_id" class="form-select">
                                ...
                                @foreach ($agencies as $a)
                                    <option value="{{ $a->id }}" @selected(old('agency_id') == $a->id)>
                                        {{ $a->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3 mx-5">
                        <div class="col-sm-6">
                            ...
                            <input type="text" name="bank_name" id="bank_name" class="form-control" required
                                value="{{ old('bank_name') }}" placeholder="e.g. HBL" />
                            @error('bank_name')
                                <span class="text-danger small">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="col-sm-6">
                            ...
                            <input type="text" name="account_title" id="account_title" class="form-control" required
                                value="{{ old('account_title') }}" placeholder="Account title" />
                            @error('account_title')
                                <span class="text-danger small">{{ $message }}</span>
                            @enderror
                        </div>
