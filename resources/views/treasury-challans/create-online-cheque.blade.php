        ...
        <div class="card mb-4" style="padding-inline: 0; border-radius: 18px 18px 9px 9px;">
            ...
            <div class="card-header-table">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <h5 class="h5-reset-margin">Create Online Cheque</h5>
                </div>
            </div>

            {{-- Body --}}
            <form method="POST" action="{{ route('billings.treasury-challans.storeOnlineCheque') }}"
                    ...
                    <div class="row g-4">
                        <div class="col-md-12">
                            <div class="row g-1">
                                ...
                                {{-- Department Name --}}
                                <div class="col-md-4">
                                    <label class="form-label-x col-form-label" for="onlineDepartmentId">Department Name</label>
                                    <select name="department_id" id="onlineDepartmentId" class="form-control select2">
                                ...
                                {{-- Office Name --}}
                                <div class="col-md-4">
                                    <label class="form-label-x col-form-label" for="onlineOfficeId">Office Name</label>
                                    <select name="office_id" id="onlineOfficeId" class="form-control select2">
                                ...
                                {{-- INF Number --}}
                                <div class="col-md-4">
                                    <label class="form-label-x col-form-label" for="onlineInfNo">INF Number</label>
                                    <select name="inf_number[]" id="onlineInfNo" class="form-control select2" multiple>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="hidden" id="onlineTotalBill" name="newspapers_amount" class="form-control"
                                        value="0" readonly />
                                </div>

                                {{-- Total Amount --}}
                                <div class="col-md-4">
                                    <label class="form-label-x col-form-label" for="onlineTotalBillAmount">Cheque Amount</label>
                                    <input type="text" id="onlineTotalBillAmount" name="total_amount"
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            $("#onlineDepartmentId").on("change", function() {
                let departmentId = $(this).val();

                if (departmentId) {
                    $.ajax({
                        url: "{{ route('get.online.offices.by.department') }}",
                        type: "GET",
                        data: {
                            departmentId: departmentId
                        },
                        success: function(data) {
                            let $officeSelect = $("#onlineOfficeId");
                            $officeSelect.empty();
                            $officeSelect.append(
                                '<option value="" disabled selected>Select Office</option>');
    ...
    <script>
        ...
        $(document).ready(function() {
            function onlineFetchInfNumbers() {
                let officeId = $("#onlineOfficeId").val();
                let departmentId = $("#onlineDepartmentId").val();

                $.ajax({
                    url: "{{ route('get.online.inf.numbers') }}",
                    type: "GET",
                    data: {
                        officeId: officeId,
                        departmentId: departmentId
            ...
            function onlineFetchTotalBill() {
                let infNumbers = $("#onlineInfNo").val();
                let officeId = $("#onlineOfficeId").val();
                let departmentId = $("#onlineDepartmentId").val();

                if (!infNumbers || infNumbers.length === 0) {
                    $("#onlineTotalBill").text("0");
                    $("#onlineTotalBillAmount").text("0");
                    // $("#newspapers-amount").text("0"); // hidden input
                    return;
                }

                $.ajax({
                    url: "{{ route('get.online.total.bill') }}",
                    type: "GET",
                    data: {
                    ...
                    success: function(data) {
                        $("#onlineTotalBill").val(data.total);
                        $("#onlineTotalBillAmount").val(data.total);
            ...
            $("#onlineOfficeId").on("change", onlineFetchInfNumbers);
            $("#onlineDepartmentId").on("change", function() {
                if (!$("#onlineOfficeId").val()) onlineFetchInfNumbers();
            });

            // Trigger total bill fetch on INF number change
            $("#onlineInfNo").on("change", onlineFetchTotalBill);

            // Load once on page ready
            // onlineFetchInfNumbers();
        });
    </script>
