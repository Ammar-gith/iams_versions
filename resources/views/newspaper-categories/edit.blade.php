...
@push('content')
    ...
    <div class="row">
        <div class="card" style="padding-inline: 0; border-radius: 18px 18px 9px 9px;">
            <form action="{{ route('newspaperCategory.update', $newspaper_category->id) }}" method="POST" class="card-body" enctype="multipart/form-data" style="padding: 0;">
                @csrf

                {{-- Title (Header) --}}
                <div class="form-header flex w-full">
                    <h5 class="h5-reset-margin">Update Newspaper Category</h5>
                </div>
                    ...
                    <div class="row mb-3 mx-5  ">
                        <div class="col-sm-6">
                            ...
                            <input type="text" name="title" id="alignment-title" class="form-control" required
                                value="{{ $newspaper_category->title }}" />
                        </div>
                        @error('title')
