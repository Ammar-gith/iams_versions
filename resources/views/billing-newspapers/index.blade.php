        ...
        <div class="card mb-4" style="padding: 0;">
            <div class="card-header col-md-12 d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    ...
                    <h5 class="mb-0">Newspaper Bills Request</h5>
                </div>
                ...
                <div class="d-flex justify-content-end mb-1">
                    <a href="{{ route('reports.billing.export.excel', array_merge(request()->query(), ['billing_type' => 'newspaper'])) }}"
                        class="custom-excel-button me-2">
                        Export Excel</a>
                    <a href="{{ route('reports.billing.export.pdf', array_merge(request()->query(), ['billing_type' => 'newspaper'])) }}"
            ...
            <div class="modal fade" id="advancedFilterModal" tabindex="-1" aria-labelledby="advancedFilterModalLabel"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form method="GET" action="{{ route('billings.newspapers.index') }}">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    <label for="classified_ad_type_id" class="form-label">Category</label>
                                    <select name="classified_ad_type_id" id="classified_ad_type_id"
                                        class="form-select select2">
                                        <option value="">All Categories</option>
                                        @foreach ($classifiedAdTypes as $type)
                                            <option value="{{ $type->id }}"
                                                {{ request('classified_ad_type_id') == $type->id ? 'selected' : '' }}>
                                                {{ $type->type }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="modal-footer">
                                <a href="{{ route('billings.newspapers.index') }}" class="btn btn-secondary">Reset</a>
                    ...
                    <table class="table w-100 js-local-filter-table">
                        ...
                        <tbody class="table-border-bottom-0 js-local-filter-rows">
                            @foreach ($billClassifiedAds as $key => $billClassifiedAd)
                                {{-- {{ $billClassifiedAd->publication_date }} --}}
                                <tr>
                                    <td>{{ ++$key }}</td> <!-- Serial Number -->
                                    <td>{{ $billClassifiedAd->inf_number ?? '' }}</td>
                                    <td style="width:50%; white-space: normal; word-wrap: break-word;">
                                        {{ \Illuminate\Support\Str::words($billClassifiedAd->office->ddo_name ?? '', 500, '...') }}
                                    </td>
                                    <td>{{ $billClassifiedAd->classified_ad_type->type ?? '' }}</td>
                                    <td>{{ \Carbon\Carbon::parse($billClassifiedAd->publish_on_or_before)->toFormattedDateString() }}
                                    </td>
                                    <td class="text-center align-middle">
                                        <div class="d-flex justify-content-center align-items-center">
                                            ...
                                            <div class="action-item custom-tooltip">
                                                <a
                                                    href="{{ route('billings.newspapers.bill.detail', $billClassifiedAd->id) }}">

                                                    <i class='bx bx-file fs-4 bx-icon'></i>
                                                    {{-- <span class="bx-icon test-sm">Newspaper Bills</span> --}}
                                                </a>
                                                <span class="tooltip-text">Newspaper Bills</span>
                                            </div>


                                            {{-- Edit (media form) --}}
                                            <div class="action-item custom-tooltip">
                                                <a href="{{ route('billings.newspapers.print', $billClassifiedAd->id) }}"
                                                    target="_blank">
                                            ...
                                            <div class="action-item custom-tooltip">
                                                @if ($user->hasRole(['Superintendent']) && $billClassifiedAd->bill_submitted_to_role_id == null)
                                                    <form
                                                        action="{{ route('billings.newspapers.newspaperBillSubmission', $billClassifiedAd->id) }}"
                                                        method="POST">
                                                        @csrf
                                            ...
                                            {{-- submit online cheque --}}

                                            {{-- @if ($user->hasrole('Client Office'))
                                                <div class="action-item custom-tooltip">
                                                    <a
                                                        href="{{ route('billings.treasury-challans.createOnlineCheque', $billClassifiedAd->id) }}">
                                                        <i class='bx bx-right-top-arrow-circle bx-icon'></i>
                                                    </a>
                                                    <span class="tooltip-text">Submit cheque</span>
                                                </div>
                                            @endif --}}
            ...
            @else
                <div class="text-center text-muted py-4">No Bills to show</div>
            @endif
        </div>
    </div>
    {{-- ! / Page Content --}}
